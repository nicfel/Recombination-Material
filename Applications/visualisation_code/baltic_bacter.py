...
class tree: ## tree class
    ...
    def subtree(self,k=None,traverse_condition=None,fix_nodes=True):
        ...
        if subtree is None: ## nothing was traversed, return None
            return None
        else: ## something was traversed
            ...
            if fix_nodes==True and len([k for k in subtree if k.branchType=='leaf'])==0:
                return None ## want to fix hanging nodes, but no leaves were encountered, return None
            else:
    ...
    def sortBranches(self,descending=True,verbose=False):
        ...
        self.drawTree(verbose=verbose) ## update x and y positions of each branch, since y positions will have changed because of sorting

    def drawTree(self,order=None,verbose=False):
        """ Find x and y coordinates of each branch. """
        if order==None:
            order=self.traverse_tree(include_condition=lambda k:k.branchType=='leaf' or (k.branchType=='node' and len(k.children)==0)) ## order is a list of tips recovered from a tree traversal to make sure they're plotted in the correct order along the vertical tree dimension
            if verbose==True:
        ...
        name_order=[x.index for x in order]
        skips=[x.width if hasattr(x,'width') else 1 for x in order]
        ...
        while len(drawn)!=len(self.Objects): # keep drawing the tree until everything is drawn
            ...
            for k in filter(lambda w:w.index not in drawn,self.Objects): ## iterate through objects that have not been drawn
                ...
                if k.branchType=='leaf': ## if leaf - get position of leaf, draw branch connecting tip to parent node
                    ...
                    y_idx=name_order.index(k.index) ## y position of leaf is given by the order in which tips were visited during the traversal
    ...
    def getExternal(self,secondFilter=None):
        return list(filter(secondFilter,filter(lambda k:k.branchType=='leaf',self.Objects)))

    def getInternal(self,secondFilter=None):
        return list(filter(secondFilter,filter(lambda k:k.branchType=='node',self.Objects)))
    ...
    def plotPoints(self,ax,x_attr=lambda k:k.height,y_attr=lambda k:k.y,target=lambda k:isinstance(k,leaf),size_function=lambda k:40,colour_function=lambda k:'k',zorder_function=lambda k: 100,**kwargs):
        for k in filter(target,self.Objects):
...
def make_tree(data,ll=None,verbose=False):
    ...
    while i < len(data): ## while there's characters left in the tree string - loop away
        ...
        # cerberus=re.match('[\(,](#[A-Za-z0-9]+)(\[&[A-Za-z\=\,\-0-9\.\_\{\} \"]+\])',data[i-1:i+200]) ## look for beginning of reticulate branch

        if cerberus is not None:
            if verbose==True:
                print('%d adding outgoing reticulation branch %s'%(i,cerberus.group(1)))
            ...
            if destination: ## identified destination of this branch
                if verbose==True:
                    print('identified %s destination'%(cerberus.group(1)))
                ...
                if verbose==True:
                    print('destination of %s not identified yet'%(cerberus.group(1)))
            i+=len(cerberus.group())-1

        cerberus=re.match('\)(#[A-Za-z0-9]+)',data[i-1:i+200]) ## look for landing point of reticulate branch
        if cerberus is not None:
            if verbose==True:
                print('%d adding incoming reticulation branch %s'%(i,cerberus.group(1)))
            ...
            if origin: ## identified origin
                if verbose==True:
                    print('identified %s origin'%(cerberus.group(1)))
                ...
                if verbose==True:
                    print('origin of %s not identified yet'%(cerberus.group(1)))
            i+=len(cerberus.group())-1
