...
h = fopen('rates.csv', 'w');fprintf(h, 'run,Ne,recombination,relRate1,relRate2\n');
h2 = fopen('rates_sequences.csv', 'w');fprintf(h2, 'run,k,m,g,f1,f2,f3,f4\n');
...
for i = 1 : nr_reps
    ...
    fprintf(h, '%d,%.12f,%.12f,%.12f,%.12f\n', i, Ne, reassortment,relRate(1),relRate(2));
    for j = 1 : length(kappa)
        ...
        fprintf(h2, '%d,%.12f,%.12f,%.12f%s\n', i, kappa(j), mu(j),gamma(j), sprintf(',%.4f', freq(j,:)));
    end
    ...
    for r = 1 : 3
        ...
        g = fopen(sprintf('inference/inf_hky_%d_rep%d.xml', i, r), 'w');
