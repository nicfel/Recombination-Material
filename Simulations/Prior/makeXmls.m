...
%         gamma = exprnd(1);
    ...
    prior_scale = [0.2 1 5 0.2 5];
    % make 3 replicates
    sigma_vals = [0.5 0.5 0.5 2 2];
    for r = 1 : length(prior_scale)
        ...
        while ~feof(f_inf)
            ...
            elseif contains(line, 'insert_recomb_prior')
                mu_rea_prior = log(mean_rea*prior_scale(r)) - sigma_rea^2/2;
                disp(r)
                disp(mu_rea_prior);
                
                fprintf(g, '\t\t\t\t\t<distr spec="beast.math.distributions.LogNormalDistributionModel" M="%f" S="%f"/>\n', mu_rea_prior, sigma_vals(r));
